...
// @license          MIT
// @icon             https://raw.githubusercontent.com/adamlui/userscripts/master/chatgpt/bravegpt/media/images/bravegpt-icon48.png
// @icon64           https://raw.githubusercontent.com/adamlui/userscripts/master/chatgpt/bravegpt/media/images/bravegpt-icon64.png
// @compatible       chrome
// @compatible       firefox
// @compatible       edge
// @compatible       opera
...
(function() {
    ...
    async function getAnswer(question, callback) {
        ...
        function onLoad(event) {
            if (event.status === 429) {
                braveGPTdiv.innerHTML = '<p>ChatGPT is flooded with too many requests. Check back later!</p>' }
            if (event.responseText) {
                try {
                    var responseJSON = JSON.parse(event.responseText)
                    var answer = responseJSON.choices[0].message.content
                    show(answer)
                } catch (error) { console.error('Failed to parse response JSON: ', error) }
            }}
    }
    ...
    async function main() {
        ...
        getAnswer(new URL(window.location.href).searchParams.get('q')) // get/show answer
    }
