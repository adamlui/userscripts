...
(async () => {
    ...
    const message = {
        ...
        get: {
            lastMoreOptionsBtn() {
                const moreOptionsBtns = document.querySelectorAll(chatgpt.selectors.btns.moreOptions),
                      lastMoreOptionsBtns = moreOptionsBtns[moreOptionsBtns.length -1]
                return !lastMoreOptionsBtns?.closest('article') ? null : lastMoreOptionsBtns
            },
        ...
        outline: {
            hide(targetNode) { if (!targetNode) return ; targetNode.style.cssText = '' },

            show(targetNode) {
                if (!config.outlineText || !targetNode) return
                message.outline.durations ||= { iniDelay: 500 /* ms */, pulse: 5 /* s */ }
                message.outline.color ||= chatgpt.isDarkMode() ? 'rgba(223, 225, 227, 0.7)'
                                                               : 'rgba(0, 157, 255, 0.7)'
                setTimeout(() => targetNode.style.cssText = `
                    box-shadow: 0 0 0 0 ${message.outline.color} ;
                    animation: pulse-glow ${message.outline.durations.pulse}s ease-in-out ; border-radius: 8px`,
                message.outline.durations.iniDelay)
                ;(message.outline.pulseGlowKeyframes ||= dom.create.style()).textContent = `
                    @keyframes pulse-glow {
                        0%   { box-shadow: 0 0 0 0   ${message.outline.color} }
                        50%  { box-shadow: 0 0 0 8px rgba(59, 130, 246, 0) }
                        100% { box-shadow: 0 0 0 0   rgba(59, 130, 246, 0) }
                    }`
                if (!message.outline.pulseGlowKeyframes.isConnected)
                    document.head.append(message.outline.pulseGlowKeyframes)
            }
        },
    ...
    function syncConfigToUI(options) {
        if (/autoTalkDisabled|toggleHidden/.test(options.updatedKey)) {
            ...
            if (options.updatedKey == 'autoTalkDisabled')
                message[config.autoTalkDisabled ? 'stopPlaying' : 'playLast']()
        }
        ...
        sidebar: {
            ...
            stylize() { // requires lib/<chatgpt|dom>.js + env
                ...
                this.styles ||= dom.create.style(
                  ...
                  + `.${this.class} { /* parent div */
                    ...
                    .${this.class}.toggle-disabled > img { transform: scaleY(0.6) } /* shrink navicon on toggle off */
            ...
            update: {
                ...
                state() {
                    ...
                    toggles.sidebar.div.classList[
                        toggles.sidebar.toggleInput.checked ? 'remove' : 'add']('toggle-disabled')
    ...
    new MutationObserver(mutations => {
        ...
        if (locationPath != location.pathname) { // nav'd to diff page, play last msg if convo page
            locationPath = location.pathname ; if (/\/c\/[\w-]+$/.test(locationPath)) message.playLast()
        } else for (const { addedNodes } of mutations) for (const node of addedNodes) { // look for options btns created
            if (node.tagName != 'DIV'
                || !node.querySelector(chatgpt.selectors.btns.moreOptions)?.closest('article') // ignore convo options
            ) continue
            message.playLast()
        }
