    ...
    const message = {
        ...
        outline(targetNode) {
            ...
            const durations = { iniDelay: 500 /* ms */, pulse: 5 /* s */ },
                  outlineColor = chatgpt.isDarkMode() ? 'rgba(223, 225, 227, 0.7)' : 'rgba(0, 157, 255, 0.7)'
        ...
        playLast() {
            ...
            dom.get.loadedElem(chatgpt.selectors.btns.play, { timeout: 1000 }).then(playBtn => {
                playBtn.click() ; document.querySelector('textarea')?.click() ; this.outline(this.get.talkingDiv()) })
        },
        ...
        async stopPlaying() {
            ...
            if (actionBtn?.matches?.(chatgpt.selectors.btns.stop)) {
                ...
                if (config.outlineText) { // rid outline
                    const talkingMsgDiv = this.get.talkingDiv() ; if (talkingMsgDiv) talkingMsgDiv.style.cssText = '' }
            }
