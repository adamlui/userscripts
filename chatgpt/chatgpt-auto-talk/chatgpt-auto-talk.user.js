...
(async () => {
    ...
    new MutationObserver(mutations => {
        if (config.autoTalkDisabled
            || navigator.userActivation?.hasBeenActive // auto-play not allowed
            || location.search == '?temporary-chat=true' // fails to play msg
            || document.querySelector('[data-testid*=login]') // no Play btn
        ) return
        for (const mutation of mutations)
            mutation.type == 'childList' && mutation.addedNodes.forEach(node => {
                if (node?.tagName == 'DIV' && node.querySelector(btnSelectors.moreOptions)?.closest('article')) {
