...
// @require             https://cdn.jsdelivr.net/gh/adamlui/userscripts@797b7c4/assets/js/lib/css.js/dist/css.min.js#sha256-PlFAv9kfe+NAw4msoXELNG7/DngN9vwARGDEYu38FVk=
// @require             https://cdn.jsdelivr.net/gh/adamlui/userscripts@797b7c4/assets/js/lib/dom.js/dist/dom.min.js#sha256-yoPPRE+q957dP94zyZRjJKZz2TWW5ztIQiTGsBmPSww=
...
// @noframes
// @downloadURL         https://raw.githubusercontent.com/adamlui/chatgpt-auto-talk/refs/heads/main/greasemonkey/chatgpt-auto-talk.user.js
// @updateURL           https://raw.githubusercontent.com/adamlui/chatgpt-auto-talk/refs/heads/main/greasemonkey/chatgpt-auto-talk.user.js
// @homepageURL         https://github.com/adamlui/chatgpt-auto-talk/#readme
...
(async () => {
    ...
    window.app = {
        ...
        commitHashes: { app: 'bd132b8' } // for cached <app|messages>.json
    }
    ...
    Object.assign(chatgpt.selectors, {
        btns: {
            ...chatgpt.selectors.btns,
            moreOptions: 'button:has(> svg > use[href$=".svg#f6d0e2"])',
            play: 'div[data-testid*=voice-play]:has(svg > use[href$=".svg#54f145"])',
            stop: 'div[data-testid*=voice-play]:has(svg > use[href$=".svg#f64f60"])'
        },
    ...
    const settings = {
        ...
        controls: { // displays top-to-bottom in toolbar menu
            ...
            playNewRepliesOnly: { type: 'toggle', defaultVal: false,
                label: app.msgs.menuLabel_playNewRepliesOnly, helptip: app.msgs.helptip_playNewRepliesOnly }
        },
    ...
    const message = {
        ...
        get: {
            lastMoreOptionsBtn() {
                const moreOptionsBtns = document.querySelectorAll(chatgpt.selectors.btns.moreOptions),
                      lastMoreOptionsBtn = moreOptionsBtns[moreOptionsBtns.length -1]
                return !lastMoreOptionsBtn?.closest('article') ? null : lastMoreOptionsBtn
            },

            talkingDiv() {
                return this.lastMoreOptionsBtn()?.closest('article')?.querySelector(chatgpt.selectors.replyDiv)
            }
        },

        openLastMoreOptionsMenu() {
            this.get.lastMoreOptionsBtn()?.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }))
        },
        ...
        outline: {
            ...
            show(targetNode) {
                ...
                setTimeout(() => targetNode.style.cssText = `
                    box-shadow: 0 0 0 0 ${this.color} ;
                    animation: pulse-glow ${this.durations.pulse}s ease-in-out ; border-radius: 8px`,
                this.durations.iniDelay)
                if (!this.pulseGlowKeyframes?.isConnected)
                    document.head.append(this.pulseGlowKeyframes ||= dom.create.style())
                this.pulseGlowKeyframes.textContent = `@keyframes pulse-glow {
                    0%   { box-shadow: 0 0 0 0   ${this.color} }
        ...
        playLast() {
            this.openLastMoreOptionsMenu()
            dom.get.loadedElem(chatgpt.selectors.btns.play, { timeout: 1000 }).then(playBtn => {
                if (!playBtn) return
                playBtn.click() ; document.querySelector('textarea')?.click()
                this.outline.show(this.get.talkingDiv())
            })
        },
        ...
        async stopPlaying() {
            ...
            const actionBtn = await Promise.race([
                dom.get.loadedElem(chatgpt.selectors.btns.stop),
                dom.get.loadedElem(chatgpt.selectors.btns.play),
    ...
    function syncConfigToUI({ updatedKey } = {}) {
        if (/autoTalkDisabled|toggleHidden/.test(updatedKey)) {
            ...
            if (updatedKey == 'autoTalkDisabled')
                message[app.config.autoTalkDisabled || app.config.playNewRepliesOnly ? 'stopPlaying' : 'playLast']()
    ...
    const toggles = {
        ...
        sidebar: {
            ...
            stylize() { // requires lib/<chatgpt|dom>.js + env
                ...
                this.styles ||= dom.create.style(
                  ...
                  + `.${this.class} { /* parent div */
                    ...
                    .${this.class}:has(span.disabled) > img { /* shrink navicon on toggle off */
            ...
            update: {
                ...
                navicon({ preload } = {}) {
                    const baseURL = `${
                        app.urls.resourceHost.replace(/@\w+/, '@397297d')}/assets/images/icons/soundwave/tall`
                    const schemeMap = { light: 'black', dark: 'white' }
                    if (preload)
                        Object.keys(schemeMap).forEach(scheme =>
                            new Image().src = `${baseURL}/${schemeMap[scheme]}.svg`)
                    else toggles.sidebar.navicon.src = `${baseURL}/${schemeMap[env.ui.scheme]}.svg`
                },
    ...
    const playObserver = new MutationObserver(mutations => {
        ...
        if (locationPath != location.pathname) { // nav'd to diff page, play last msg if applicable
            ...
            if ( // ignore all nodes except msg option parents
                node.tagName == 'DIV' && node.querySelector(chatgpt.selectors.btns.moreOptions)?.closest('article')
            ) message.playLast()
        }
    })
    ...
    // Add KEY LISTENER to stop playing audio on ESC
    document.addEventListener('keydown', event => {
        if ((event.key.startsWith('Esc') || event.keyCode == 27)) message.stopPlaying() })
