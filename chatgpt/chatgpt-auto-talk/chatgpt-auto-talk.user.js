...
(async () => {
    ...
    window.app = {
        ...
        commitHashes: { app: '698219e' } // for cached <app|messages>.json
    }
    ...
    const playObserver = new MutationObserver(mutations => {
        ...
        if (locationPath != location.pathname) { // nav'd to diff page, play last msg if convo page
            ...
            if (!config.playNewRepliesOnly && /\/c\/[\w-]+$/.test(locationPath)) message.playLast()
            playObserver.disconnect() // to wait for More Options btns to load
