...
(async () => {
    ...
    const message = {
        ...
        openLastMoreOptionsMenu() {
            this.get.lastMoreOptionsBtn()
                ?.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }))
        },

        outline() {
            if (!config.outlineText) return
            const talkingMsgDiv = this.get.talkingDiv() ; if (!talkingMsgDiv) return
            const durations = { iniDelay: 500 /* ms */, pulse: 5 /* s */ }
            const outlineColor = chatgpt.isDarkMode() ? 'rgba(223, 225, 227, 0.7)' : 'rgba(0, 157, 255, 0.7)'
        ...
        playLast() {
            ...
            dom.get.loadedElem(chatgpt.selectors.btns.play, { timeout: 1000 }).then(playBtn => {
                playBtn.click() ; document.querySelector('textarea')?.click() ; this.outline() })
        },
