...
(async () => {
    ...
    function getTalkingMsgDiv() {
        return getLastMoreOptionsBtn()?.closest('article')?.querySelector(chatgpt.selectors.replyDiv)
    }

    function openLastMoreOptionsMenu() {
        const lastMoreOptionsBtn = getLastMoreOptionsBtn() ; if (!lastMoreOptionsBtn) return
    ...
    async function stopPlayingMsg() {
        ...
        if (actionBtn?.matches?.(chatgpt.selectors.btns.stop)) {
            ...
            if (config.outlineText) { // rid outline
                const talkingMsgDiv = getTalkingMsgDiv() ; if (talkingMsgDiv) talkingMsgDiv.style.cssText = '' }
        }
