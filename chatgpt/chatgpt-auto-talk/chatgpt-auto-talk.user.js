...
(async () => {
    ...
    window.app = {
        ...
        commitHashes: { app: 'ffd2892' } // for cached <app|messages>.json
    }
    ...
    const moreOptionsBtnSelector = 'button:has(path[d^="M15.498"])'
    const autoPlayObserver = new MutationObserver(mutations => {
        if (config.autoTalkDisabled || location.search == '?temporary-chat=true'
            || document.querySelector('[data-testid*=login]')
        ) return
        for (const mutation of mutations)
            mutation.type == 'childList' && mutation.addedNodes.forEach(node => {
                if (node?.tagName == 'DIV' && node.querySelector(moreOptionsBtnSelector)?.closest('article')) {
                    const moreOptionsBtns = document.querySelectorAll(moreOptionsBtnSelector),
                    ...
                    dom.get.loadedElem('div[data-testid=voice-play-turn-action-button]').then(playBtn => {
