...
(async () => {
    ...
    new MutationObserver(mutations => {
        ...
        for (const { addedNodes } of mutations) for (const node of addedNodes) {
            if (node.tagName != 'DIV'
                || !node.querySelector(btnSelectors.moreOptions)?.closest('article') // ignore convo options
            ) continue
