...
(async () => {
    ...
    function getLastMoreOptionsBtn() {
        const moreOptionsBtns = document.querySelectorAll(app.selectors.btns.moreOptions),
    ...
    function getTalkingMsgDiv() {
        return getLastMoreOptionsBtn()?.closest('article')?.querySelector(app.selectors.replyDiv)
    }
    ...
    function playLastMsg() {
        ...
        dom.get.loadedElem(app.selectors.btns.play, { timeout: 1000 }).then(playBtn => {
    ...
    async function stopPlayingMsg() {
        ...
        const actionBtn = await Promise.race([
            dom.get.loadedElem(app.selectors.btns.stop),
            dom.get.loadedElem(app.selectors.btns.play),
            new Promise(resolve => setTimeout(resolve, 1000))
        ])
        if (actionBtn?.matches?.(app.selectors.btns.stop)) {
    ...
    new MutationObserver(mutations => {
        ...
        if (locationPath != location.pathname) { // nav'd to diff page, play last msg if convo page
            ...
            if (node.tagName != 'DIV'
                || !node.querySelector(app.selectors.btns.moreOptions)?.closest('article') // ignore convo options
            ) continue
            playLastMsg()
        }
