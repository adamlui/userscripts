...
(async () => {
    ...
    const app = {
        ...
        latestAssetCommitHash: 'e5789e4' // for cached messages.json
    }
    ...
    const modals = {
        ...
        about: {
            show() {
                ...
                aboutHeaderLogo.style.cssText = `max-width: 98% ; margin: -1px ${ env.browser.isMobile ? 'auto' : '13.5%' } 1px`
    ...
    const logos = {
        ddgpt: {
            ...
            update(...targetLogos) {
                ...
                targetLogos.forEach(logo => logo.src = GM_getResourceText(`ddgpt${ ui.app.scheme == 'dark' ? 'DS' : 'LS' }logo`))
            }
        }
    }
        ...
        style: {
            app() {
                ...
                appStyle.innerText = (
                  ...
                  + `#ddgpt pre a:hover { color: ${ ui.app.scheme == 'dark' ? 'white' : '#ea7a28' }}`
                  ...
                  + `.modal-buttons { margin: 24px -5px -3px ${ env.browser.isMobile ? -5 : -15 }px !important ; width: 100% }` // pos/size modal buttons
                  ...
                  + '#ddgpt-settings li {'
                      ...
                      + `padding: 4px 10px ; border-bottom: 1px dotted ${ ui.app.scheme == 'dark' ? 'white' : 'black' } ;` // add settings separators
    ...
    const show = {
        ...
        reply(answer) {
            ...
            if (!appDiv.querySelector('pre')) {
                ...
                if (!ui.site.isCentered && !env.browser.isMobile) {
                ...
                // Create/append Wider Sidebar button
                    ...
                    wsbBtn.className = 'corner-btn' ; wsbBtn.style.margin = `${ env.browser.isFF ? 0.5 : 0 }px 13.5px 0 0`
                        ...
                        for (const [groupName, adGroup] of shuffle(applyBoosts(Object.entries(campaign.adGroups)))) {
                            ...
                            if (/^self$/i.test(groupName) && !re_appName.test(campaignName) // self-group for other apps
                                ...
                                || adGroup.targetLocations && ( // target locale(s) exist...
                                    !env.userLocale || !adGroup.targetLocations.some( // ...and user locale is missing or excluded
